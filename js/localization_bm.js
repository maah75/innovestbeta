...
function localize(txt, language){
    ...
    switch(language){
        ...
        lclzedText = "About US";
        if(txt == "aboutusP")
        lclzedText = "منتج سكني عصري متكامل تم وضع استدامة المسكن نصب آعيينا. صمم بعناية لخلق مجتمع حيوي في شمال الأحساء تقدمه لكم بفخر شركة الإستثمارات المبتكرة للتطوير العقاري ( انوفست العقارية). نطمح لتوفير حياة عصرية ومتطورة في بيئة هادئة تلائم خصوصية وتقاليد مجتمعنا";
        if(txt == "housingUnits")
        lclzedText = "Number of housing units";
        if(txt == "housingUnitsNumber")
        lclzedText = "192 Units";
        ...
        lclzedText = "Location";
        if(txt == "LocationD")
        lclzedText = "The location is 5 minutes from the heart of Al Ahsa";
        if(txt == "EnergySaving")
        ...
        lclzedText = "الموقع";
        if(txt == "LocationD")
        lclzedText = "يبعد الموقع ٥ دقاىق عن قلب الاحساء النابض";
        if(txt == "EnergySaving")
